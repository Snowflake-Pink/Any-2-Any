...
import hashlib
import os
from utils.parser import parse_nature_code_to_code, parse_code_to_nature_code, parse_code_to_workflow
...
from inference_engine.pseudo_natural.inference_engine.analyzer import get_analyzer_inference_engine_prompt, parse_analyzer_inference_engine_response
from inference_engine.pseudo_natural.inference_engine.node_generator import get_generator_inference_engine_prompt, parse_generator_inference_engine_response
from inference_engine.pseudo_natural.inference_engine.linker import (
    get_linker_inference_engine_prompt,
    parse_linker_inference_engine_response,
    get_node_knowledge
)
from inference_engine.pseudo_natural.inference_engine.refiner import get_refiner_inference_engine_prompt, parse_refiner_inference_engine_response
import yaml
...
class PseudoNaturalPipeline:
    def __init__(
        ...
        self.key_nodes = parse_code_to_nature_code(key_nodes)
    ...
    def __call__(self, query_text: str):
        ...
        code = parse_code_to_nature_code(code)
        ...
        for ref in references:
            ...
            reference_code = parse_code_to_nature_code(reference_code)
            reference += f'<code>\n{reference_code}\n</code>\n\n'
        ...
        try:
            ...
            if "create nodes by instantiation" in linker_code.lower() or "instantiate nodes" in linker_code.lower():
        ...
        try:
            code = parse_nature_code_to_code(code)
    ...
    def _run_refiner(self, query_text: str, code: str, description:str, error_message: str, reference: str):
        for fix_epoch in range(self.num_fixes + 1):
            ...
            try:
                ...
                self.logger.info(f'Parsed code:\n  {refined_code}')
                self.logger.info(f'Parsed description:\n  {refined_description}')
                code = parse_nature_code_to_code(refined_code)
