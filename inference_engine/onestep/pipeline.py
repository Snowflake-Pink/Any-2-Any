...
from inference_engine.onestep.inference_engine.analyzer import get_analyzer_inference_engine_prompt, parse_analyzer_inference_engine_response
from inference_engine.onestep.inference_engine.one_step_generator import get_one_step_generator_prompt, parse_one_step_response, get_node_knowledge

from inference_engine.onestep.inference_engine.refiner import get_refiner_inference_engine_prompt, parse_refiner_inference_engine_response
import yaml
...
class OneStepPipeline:
    ...
    def __call__(self, query_text: str):
        ...
        for ref in references:
            if ref is None:
                self.logger.Error('Invalid reference')
            ...
            try:
                ...
                self.logger.error(f'Error parsing reference code ({ref.metadata['code']}) to workflow code: {str(error)}')
        ...
        generator_message = get_one_step_generator_prompt(
            query=query_text,
            analysis=analysis,
        ...
        one_step_response = answer.content
        self.logger.info(f'Generated answer:{one_step_response}')
        self.logger.info(f'Token usage:{usage}')

        try:
            if "python" not in one_step_response:
                linker_code, description = parse_one_step_response(one_step_response)
            else:
                _ , description = parse_one_step_response(one_step_response)
                linker_code = one_step_response.split("```")[1].replace("python", "").strip()
            code = linker_code   
