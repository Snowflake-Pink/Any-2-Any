...
import os
from utils.parser import parse_wfcode_to_code
import re
one_step_prompt = '''
## Task
...
In this task, you will create a **complete ComfyUI workflow**, combining all three stages (`add`, `invoke`, and `link`) into one continuous process. The workflow must meet the user's requirements, ensuring all nodes are instantiated, invoked, and linked correctly. This ensures the generated workflow is functional, coherent, and meets the user’s input requirements.
</introduciton>
...
1. Select all Key Nodes given by the user (displayed above).
...
Other Requirements on Add Node:
1. When you add node, please make sure you initiate correct parameters.
...
5. Make sure all the key nodes are included and necessary extra nodes for the query are added.
...
10. A query ask for video generation with no stated input is a text to video query.
...
Your code must meet the following rules:
1. Each code line should either add (`add_node`) a node or invoke (`invoke_node`) a node. You should not invoke a node before it is added.
...
7. Do not use indexing to extract output variables. Instead, assign the outputs directly based on the node’s output type. For example, `output_2 = node_1()[1]` is not allowed and should be `output_1, output_2 = node_1()`.
8. Verify that the input and output types of linked nodes are compatible. If there is a mismatch, modify the workflow by relinking nodes or adding necessary nodes to resolve the issue. For example, using `workflow.connect("clip_vision", "cliptextencode_7", "clip")` is incorrect due to a type mismatch, while `workflow.connect("clip_text", "cliptextencode_7", "clip")` is correct.
...
def get_one_step_generator_prompt(
    query: str,
    analysis: str,
    key_nodes: str,
    reference: str,
    ...
    prompt_text = one_step_prompt.format(
        query=query,
        analysis=analysis,
        key_nodes=f'<key_nodes>\n{key_nodes}\n</key_nodes>',
        reference=reference,
        key_node_knowledge=key_node_knowledge
    )
    return prompt_text

def parse_one_step_response(response: str):
...
def unsafify_var_name(name):
    return name.replace('LeftBracket', '[').replace('RightBracket', ']').replace('LeftParen', '(').replace('RightParen', ')').replace('Space',' ').replace('Plus','')
...
def get_node_knowledge(node_code, md_dir="./dataset/docs/node", strict=True):
    ...
    print(node_names)
    # print(node_names)
    all_nodes = [(name.split(".")[0].replace(" ","").replace("-","").replace("_","").replace("(","").replace(")","").replace("+","").lower(), name) for name in os.listdir(md_dir)]
    # print(all_nodes)
